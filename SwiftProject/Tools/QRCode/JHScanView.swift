...
open class JHScanView: UIView
{
    //扫码区域各种参数
    var viewStyle:JHScanViewStyle = JHScanViewStyle()
    
     //扫码区域
    var scanRetangleRect:CGRect = CGRect.zero
    
    //线条扫码动画封装
    var scanLineAnimation:JHScanLineAnimation?
    
    //网格扫码动画封装
    var scanNetAnimation:JHScanNetAnimation?
    ...
    */
    public init(frame:CGRect, vstyle:JHScanViewStyle )
    {
        viewStyle = vstyle
        
        switch (viewStyle.anmiationStyle)
        {
        case JHScanViewAnimationStyle.LineMove:
            scanLineAnimation = JHScanLineAnimation.instance()
            break
        case JHScanViewAnimationStyle.NetGrid:
            scanNetAnimation = JHScanNetAnimation.instance()
            break
        case JHScanViewAnimationStyle.LineStill:
...
//        print("frame:%@",NSStringFromCGRect(self.frame))
        ...
        {
        case JHScanViewPhotoframeAngleStyle.Outer:
                diffAngle = linewidthAngle/3//框外面4个角，与框紧密联系在一起
           
        case JHScanViewPhotoframeAngleStyle.On:
                diffAngle = 0
            
        case JHScanViewPhotoframeAngleStyle.Inner:
...
//        右下角水平线
    ...
    //根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView:UIView, style:JHScanViewStyle) -> CGRect
    {
